...
for i = 1:N
    ...
    for j = 1:numEvents
       if data{i,1}.mrk.y(2,j) == 1 % 5 indicates we're finding the target based off react_emg, 2 indicates we're finding the target based on the lead care braking
...
converge_testing = all_features{round(N*ttSplit)};% round(N*ttSplit)+1};
...
% model = fitcdiscr(Training,'Label','OptimizeHyperparameters','auto');
...
% scores = model.Prior;
% [X,Y,T,AUC] = perfcurve(actual_labels,scores,'virginica');
